...
// @namespace    https://github.com/guiguisocute/smart_srun_school_preset_capture.user
// @version      0.1.3
// @description  采集一次真实的深澜 SRun 校园网网页登录，生成可提交给 smart-srun 的 school-presets.json 预设条目。
// @author       smart-srun maintainers
// @homepageURL  https://github.com/guiguisocute/smart_srun_school_preset_capture.user
// @supportURL   https://github.com/guiguisocute/smart_srun_school_preset_capture.user/issues
// @downloadURL  https://raw.githubusercontent.com/guiguisocute/smart_srun_school_preset_capture.user/main/smart_srun_school_preset_capture.user.js
// @updateURL    https://raw.githubusercontent.com/guiguisocute/smart_srun_school_preset_capture.user/main/smart_srun_school_preset_capture.user.js
// @match        http://*/*
// @match        https://*/*
...
(function () {
  ...
  // 运营商后缀“未验证”哨兵：标签已知但真实后缀尚未抓到时使用，与预设 JSON 约定一致。
  var UNVERIFIED_SUFFIX = "??";
  ...
  function captureUsername(username, source) {
    ...
    // 累积记录本次登录用到的运营商后缀。登录即便失败也会走到这里（info 已解出 username），
    // 所以用户用不同运营商账号多试几次，就能把一所学校的运营商后缀一次性抓全。
    recordCapturedOperator(suffix);
  ...
  function buildPresetEntry() {
    var suffixOverride = String(readField("suffix_override") || "").trim();
    var operatorOverride = String(readField("operator_override") || "").trim().toLowerCase();
    ...
    // 手动填写的后缀/运营商也并入累积的运营商列表（"??" 占位除外）。
    if (suffixOverride) {
      recordCapturedOperator(suffixOverride);
    } else if (operatorOverride) {
      recordCapturedOperator(operatorOverride);
    }
    if (status === "auto") {
    ...
    operators = buildOperatorsForEntry();
  ...
  // 记录一次真实抓到的运营商后缀（空串=纯校园网账号），按后缀去重并累积保存。
  function recordCapturedOperator(suffix) {
    var sfx = String(suffix || "").trim();
    if (sfx === UNVERIFIED_SUFFIX) {
      return;
    }
    if (Object.prototype.toString.call(state.captured_operators) !== "[object Array]") {
      state.captured_operators = [];
    }
    var idx;
    for (idx = 0; idx < state.captured_operators.length; idx += 1) {
      if (String(state.captured_operators[idx].suffix || "") === sfx) {
        return;
      }
    }
    state.captured_operators.push({ suffix: sfx, label: operatorLabelFromSuffix(sfx) });
  }

  function pushUniqueOperator(operators, suffix, label) {
    var sfx = String(suffix || "").trim();
    var idx;
    for (idx = 0; idx < operators.length; idx += 1) {
      if (String(operators[idx].suffix || "") === sfx) {
        return;
      }
    }
    operators.push({ suffix: sfx, label: label || operatorLabelFromSuffix(sfx) });
  }

  // 遵循“抓一个补一个”：只导出真正抓到过的运营商后缀，不预设某学校一定有三大运营商，
  // 也不主动注入 "??" 占位（"??" 是维护者手工标注“已知存在但后缀未验证”的约定）。
  function buildOperatorsForEntry() {
    var operators = [];
    var captured = (Object.prototype.toString.call(state.captured_operators) === "[object Array]")
      ? state.captured_operators
      : [];
    var idx;
    var sfx;

    for (idx = 0; idx < captured.length; idx += 1) {
      sfx = String(captured[idx].suffix || "");
      pushUniqueOperator(operators, sfx, captured[idx].label);
    }
    // 兼容旧路径：本次抓到的“无后缀”账号还没进 captured 列表时也并入。
    if (!operators.length && state.username_has_suffix === false) {
      pushUniqueOperator(operators, "", "校园网");
    }
    return operators;
  }
  ...
  function operatorLabelFromSuffix(suffix) {
    ...
    for (idx = 0; idx < DEFAULT_OPERATORS.length; idx += 1) {
      if (DEFAULT_OPERATORS[idx].suffix === inferred) {
  ...
  // 汇总目前累积抓到的运营商后缀，用于面板提示。不预设“应该有哪些运营商”。
  function capturedOperatorsSummary() {
    var captured = (Object.prototype.toString.call(state.captured_operators) === "[object Array]")
      ? state.captured_operators
      : [];
    var done = [];
    ...
    for (idx = 0; idx < captured.length; idx += 1) {
      sfx = String(captured[idx].suffix || "");
      label = String(captured[idx].label || operatorLabelFromSuffix(sfx) || "校园网");
      done.push(label + "=" + (sfx ? sfx : "无后缀"));
    }
    return { done: done, count: captured.length };
  }

  function capturedOperatorsHintHtml() {
    var s = capturedOperatorsSummary();
    if (!s.count) {
      return "";
    }
    var doneText = s.done.join("、");
    return '<div style="margin-top:10px;padding:8px;border:1px solid #313244;border-radius:6px;background:#181825;font-size:11px;line-height:1.6;">' +
      '<div style="color:#a6e3a1;font-weight:bold;">已抓取运营商后缀（' + s.count + '）</div>' +
      '<div style="color:#cdd6f4;font-family:monospace;word-break:break-all;">' + escapeHtml(doneText) + '</div>' +
      '<div style="color:#a6adc8;margin-top:4px;">抓一个补一个：如果这所学校还有其它运营商，请换用对应运营商账号再登录一次即可补全（登录失败也能抓取）。</div>' +
      '</div>';
  }
  ...
  function mergeState(base, extra) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(base, "captured_operators") &&
      (!out.captured_operators || Object.prototype.toString.call(out.captured_operators) !== "[object Array]")
    ) {
      out.captured_operators = [];
    }
    return out;
  }
  ...
  function panelContentHtml() {
    ...
    if (state.login_success === true && formState.ui_step === "submit") {
      return '<div style="padding: 12px; color: #cdd6f4;">' +
        ...
        rowHtml("接入方式", selectHtml("access_mode", [["", "（请选择）"], ["wifi", "无线"], ["wired", "有线（WAN）"]])) +
        rowHtml("SSID", inputHtml("ssid", "无线填写校园网 SSID，有线可留空")) +
        rowHtml("贡献者", inputHtml("contributors", "@你的GitHub用户名，多个用逗号或空格分隔")) +
  ...
  function wirePanelButtons() {
    ...
    if (submitInfo) {
      submitInfo.onclick = function () {
        // status 是维护者标注的 tag，不由终端用户填写：保持 "auto"，
        // 由 buildPresetEntry 按登录是否成功自动给出 active/draft，维护者评审时再调整。
